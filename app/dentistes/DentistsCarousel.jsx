...
export default function DentistsCarousel({ dentists }) {
  const SWIPE_DURATION_MS = 440;
  const total = Array.isArray(dentists) ? dentists.length : 0;
  ...
  const cards = [
    { slot: "left", dentist: dentists[leftIndex], index: leftIndex },
    { slot: "center", dentist: dentists[activeIndex], index: activeIndex },
    { slot: "right", dentist: dentists[rightIndex], index: rightIndex },
  ];
      ...
      >
        {cards.map(({ slot, dentist, index }) => (
          ...
          >
            ...
            <p>{`Diplômé de la faculté dentaire de ${dentist.faculty}.`}</p>
            <p className="medecin-card-step">{`${index + 1}/${total}`}</p>
            <a className="medecin-rdv-link" href="tel:+33329346070">
